...
export const initShaders = (gl: WebGLRenderingContext) => {
...
const drawRectangle = (gl: WebGLRenderingContext, x: number, y: number, width: number, height: number) => {
  const x1 = x;
  const y1 = y;
  const x2 = x + width;
  const y2 = y + height;
...
export function initDrawingContext(canvas: HTMLCanvasElement) {
  ...
  return (gameState: MutableGameState) => {
    ...
    drawRectangle(gl, ballX - radius, ballY - radius, radius * 2, radius * 2);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
  };
}
